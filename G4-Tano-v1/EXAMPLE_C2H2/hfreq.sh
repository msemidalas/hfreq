...
################################
# MP2/ano-pVQZ force constants #

cat << EOF > "$myfilename".MP2.anopVQZ.com
gthresh,energy=1d-12,gradient=1d-6,twoint=1d-18,prefac=1d-20
noorient
charge=0
multiplicity=1
...
awk -v geomfile="$output_file_geom" '/geom={/ { print; while ((getline < geomfile) > 0) print; close(geomfile); next } 1' "$myfilename.MP2.anopVQZ.com" > tmp && mv tmp "$myfilename.MP2.anopVQZ.com"
...
./runmolpro-es "$myfilename".MP2.anopVQZ.com 8 1 1500
...
python3 convert_hessian_MOLPRO_to_Psi4.py "$myfilename".MP2.anopVQZ.out

python3 hessian_G4-anoT-mp2_anopVQZ.py "$myfilename"
